...
// Updates an Agenda Item's Status property in Notion (e.g. Approved / Rejected),
// used by the Approval Routing screen's Approve/Reject buttons.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!statusProp) {
      return res.status(500).json({ error: "Could not find a Status property on Agenda Items" });
    }
